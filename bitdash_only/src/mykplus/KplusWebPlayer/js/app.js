...
exports.getChannelEPG = function (channelId, language, startoverLength) {
    ...
    var minuntesMod = 0;
    if (kpAp.config.epgCallTParam.getDate() == crntSysDate.getDate()) {
    	minuntesMod = (kpAp.config.epgCallTParam.getMinutes() % kpAp.config.getAppConfigDirectResponse.web.cdn_time_interval.today);
    	kpAp.config.epgCallTParam.setMinutes(kpAp.config.epgCallTParam.getMinutes() - minuntesMod);
    }
    kpAp.config.epgCallTParam.setSeconds(0);
    var dataToBsent = {
    ...
    var urlParams = '?t='+ (kpAp.kpDash.getSystemUTCtimestamp(new Date(kpAp.config.epgCallTParam))).toString();
...
ensureFngrPrntVsblty() {
 ...
 onOverlayedLangVTselected(e) {
 	...
 	if (selecLang === 'eng') {
 		...
 		$(".lang-en-div img").attr("src", "/ottservices/KplusWebPlayer/img/buttons/check_radio_button.png");

 		$(".outer-lang-vt-div").css("background-color", "");
 		$(".lang-vt-div img").attr("src", "/ottservices/KplusWebPlayer/img/buttons/uncheck_radio_button.png");
 	} else if (selecLang === 'vie') {
 		$(".outer-lang-vt-div").css("background-color", "rgba(255,255,255,0.5)");
 		$(".lang-vt-div img").attr("src", "/ottservices/KplusWebPlayer/img/buttons/check_radio_button.png");

 		$(".outer-lang-en-div").css("background-color", "");
 		$(".lang-en-div img").attr("src", "/ottservices/KplusWebPlayer/img/buttons/uncheck_radio_button.png");
 	}
 },
...
function videoBitrateChangeWorkAround({player = {}, conf = {}} = {}) {
	  var lowestQualitySuggestionCount = 0;
	  var forceNewQualityCount = 5;
	  var forceNewQualityId = "";
	  var bufferThresholdLevel = 70;
	  var lowestQualitySuggestionThreshhold = 6;
	  function getNextHigherAvailableQuality(currentQualityId) {
		    var qualities = player.getAvailableVideoQualities();
		    var currentQuality, nextQuality;
		    for (var i = 0; i < qualities.length; i++) {
			      if (qualities[i].id === currentQualityId) {
				        currentQuality = qualities[i];
				        break;
			      }
		    }
		    for (var j = 0; j < qualities.length; j++) {
			      if (currentQuality.bitrate < qualities[j].bitrate) {
				        nextQuality = qualities[j];
				        break;
			      }
		    }
	...
	  function getLowestAvailableQuality() {
		...
		    for (var i = 0; i < qualities.length; i++) {
			      if (qualities[i].bitrate < selectedQuality.bitrate) {
				        selectedQuality = qualities[i];
			      }
		    }
		    return selectedQuality.id;
	  }

	  function getMaxBufferLevel() {
		    if (conf.hasOwnProperty("tweaks") && conf.tweaks.hasOwnProperty("max_buffer_level")) {
			      return conf.tweaks.max_buffer_level;
		    }
		    return 40;
	  }

	  function getCurrentVideoBufferLevelInPercent() {
		    return (player.getVideoBufferLength() / getMaxBufferLevel()) * 100;
	  }
	    return (details) => {
		      if (forceNewQualityId && forceNewQualityCount > 0) {
			        forceNewQualityCount--;
			        return forceNewQualityId;
		      } else {
			        forceNewQualityId = "";
			        forceNewQualityCount = 5;
		      }


      if (details.suggested === getLowestAvailableQuality()) {
	        if (getCurrentVideoBufferLevelInPercent() >= bufferThresholdLevel) {
		          lowestQualitySuggestionCount++;
	        } else {
		          lowestQualitySuggestionCount = 0;
	        }
    
        if (lowestQualitySuggestionCount >= lowestQualitySuggestionThreshhold && getCurrentVideoBufferLevelInPercent() >= bufferThresholdLevel) {
	          forceNewQualityId = getNextHigherAvailableQuality(details.suggested).id;
	          lowestQualitySuggestionCount = 0;
...
function httpRequestRetryWorkAround({player = {}, conf = {}} = {}) {
	const maxManifestRetryCount = 20;
	window.playerHttpRequestRetryCount = 0;

	  return (type, response) => {
		    if (type === bitmovin.player.network.REQUEST_TYPE.MANIFEST_DASH && window.playerHttpRequestRetryCount < maxManifestRetryCount) {
			      window.playerHttpRequestRetryCount++;
			      return new Promise(function (resolve) {
        setTimeout(function () { resolve(response.request); }, 2 * 1000); // 2 Seconds.
      });
		    }
...
bitdashConfig.adaptation = {
	desktop: {
		onVideoAdaptation: videoBitrateChangeWorkAround({
			player: bitmovin.player(this.dashContainerUniqueId),
			conf: bitdashConfig,
		})
	},
	logic: 'v3',
};
bitdashConfig.network = {
	retryHttpRequest: httpRequestRetryWorkAround({
		player: bitmovin.player(this.dashContainerUniqueId),
		conf: bitdashConfig,
	})
}
...
chkAudioLangsOfstream() {
 ...
 seekStartoverStream(ratio){
	...
	console.log(kpAp.config.bandwidthSelected);
});
...
$.fn.KPlusWebPlayer = function (options) {
    ...
    function initializeBitdashPlayer() {
    	...
    	var playerUITempate = $.ajax({
    		type: "GET",
    		timeout: 180000,
    		url: '/ottservices/KplusWebPlayer/template/bitdash-plyr-tpl.html?version=@@project-version-placeholder',
    		async: false
    	}).responseText;
    ...
    function initializeSilverLightPlayer() {
    	...
    	var silverLightTemplate = $.ajax({
    		type: "GET",
    		timeout: 180000,
    		url: '/ottservices/KplusWebPlayer/template/silverlight-plyr-tpl.html?version=@@project-version-placeholder',
    		async: false
    	}).responseText;
 	...
 	kpAp.APIcallerModule.validateToken().then(function (data) {
 		...
 		if ('Error' in data || 'errorCode' in data) {
            ...
            if (plugin.isChromeBrowser || plugin.isEdgeBrowser) {
                ...
                //Send straight to error if R is false
                if (!kpAp.runPlayer) {
                	kpAp.displayPlyrMsg(3, void 0, "CSM-1004");
                	if (bitmovin.player(this.dashContainerUniqueId).isPlaying()) {
                		bitmovin.player(this.dashContainerUniqueId).unload();
